...
from src.dmm_causal import DMM, train_dmm

def main():

    usage = "Train a causal DMM using trajectories of SSMs \n"\
        "python3.8 main_danse.py --mode [train/test] --rnn_model_type [gru/lstm/rnn] --dataset_type [LinearSSM/LorenzSSM] \n"\
    ...
    print("datafile name: {}".format(datafile.split('/')[-1]))
    ...
    batch_size = est_parameters_dict["dmm"]["batch_size"] # Set the batch size
    estimator_options = est_parameters_dict["dmm"] # Get the options for the estimator
    ...
    print(estimator_options['H'], estimator_options['C_w'])
    ...
    if mode.lower() == "train":
        ...
        model_dmm = DMM(**estimator_options)
        tr_verbose = True  
        
        # Starting model training
        tr_losses, val_losses, _, _, _ = train_dmm(
            model=model_dmm,
            train_loader=train_loader,
            val_loader=val_loader,
            options=estimator_options,
            optimizer_params=estimator_options['optimizer_params'],
        ...
        with open(os.path.join(os.path.join(logfile_path, main_exp_name), 
            'dmm_{}_{}_losses_eps{}.json'.format(estimator_options['inference_mode'], 
                                                 estimator_options['rnn_model_type'], 
                                                 estimator_options['optimizer_params']['num_epochs'],)), 'w') as f:
