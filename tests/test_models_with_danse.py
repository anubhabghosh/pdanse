...
def test_on_ssm_model(
    ...
    plot_state_trajectory_w_lims(
        ...
        X_est_UKF_std=np.sqrt(
            torch.diagonal(
                torch.squeeze(Pk_estimated_ukf[0, :, :, :], 0), offset=0, dim1=1, dim2=2
            ).numpy()
        )
        if "ukf" in models_list
        else None,
...
if __name__ == "__main__":
    ...
    test_logfile = "./log/{}_test_{}_T_{}_N_{}_ModTest.log".format(
        ssmtype, evaluation_mode, T_test, N_test
    )
    test_jsonfile = "./log/{}_test_{}_T_{}_N_{}_ModTest.json".format(
        ssmtype, evaluation_mode, T_test, N_test
    )
