...
from test_danse_ssm import test_danse_ssm, test_danse_semisupervised_ssm, test_danse_supervised_ssm
...
def test_on_ssm_model(device='cpu', learnable_model_files=None, test_data_file=None, 
    ...
    model_file_saved_knet = learnable_model_files["kalmannet"] if "kalmannet" in models_list else None
    model_file_saved_dmm = learnable_model_files["dmm_st-l"] if "dmm_st-l" in models_list else None
    ...
    if "kalmannet" in models_list:
        ...
        X_estimated_filtered_knet, time_elapsed_knet = test_kalmannet_ssm(
            Y_test=Y, f_fn=f_fn, ssm_model_test=ssm_model_test, model_file_saved_knet=model_file_saved_knet, device=device
        )
    ...
    if "dmm_st-l" in models_list:
        ...
        # Initialize the KalmanNet model in PyTorch
        X_estimated_filtered_dmm_causal, Pk_estimated_filtered_dmm_causal, time_elapsed_dmm_causal = test_dmm_causal_ssm(
            Y_test=Y, ssm_model_test=ssm_model_test, Cw_test=Cw, model_file_saved_dmm=model_file_saved_dmm, device=device
        )
        ...
        X_estimated_filtered_dmm_causal, Pk_estimated_filtered_dmm_causal, time_elapsed_dmm_causal = None, None, None
    #####################################################################################################################################################################
    ...
    for metric_name in metrics_list:
        ...
        if metric_fn is not None:
            metrics_dict_for_one_smnr[metric_name] = {
                ...
                "danse_supervised": metric_fn(X[:,:,:], X_estimated_filtered_danse_supervised[:,0:,:]).numpy().item() if "danse_supervised" in models_list else None,
                "danse_semisupervised": metric_fn(X[:,:,:], X_estimated_filtered_danse_semisupervised[:,0:,:]).numpy().item() if "danse_semisupervised" in models_list else None,
                "dmm_st-l": metric_fn(X[:,:,:], X_estimated_filtered_dmm_causal[:,0:,:]).numpy().item() if "dmm_st-l" in models_list else None,
                "kalmannet": metric_fn(X[:,:,:], X_estimated_filtered_knet[:,0:,:]).numpy().item() if "kalmannet" in models_list else None
            }
    ...
    metrics_dict_for_one_smnr["time_elapsed"] = {
                ...
                "danse_supervised": time_elapsed_danse_supervised if "danse_supervised" in models_list else None,
                "danse_semisupervised": time_elapsed_danse_danse_semisupervised if "danse_semisupervised" in models_list else None,
    ...
    plot_state_trajectory_axes_all(
        ...
        X_est_SemiDANSE=torch.squeeze(X_estimated_filtered_danse_semisupervised[0],0).numpy() if "danse_semisupervised" in models_list else None,
        X_est_DMM=torch.squeeze(X_estimated_filtered_dmm_causal[0],0).numpy() if "dmm_st-l" in models_list else None,
        X_est_KNET=torch.squeeze(X_estimated_filtered_knet[0], 0).numpy() if "kalmannet" in models_list else None,
        savefig=True,
    ...
    plot_state_trajectory_w_lims(
        ...
        X_est_SemiDANSE=torch.squeeze(X_estimated_filtered_danse_semisupervised[0],0).numpy() if "danse_semisupervised" in models_list else None,
        X_est_SemiDANSE_std=np.sqrt(torch.diagonal(torch.squeeze(Pk_estimated_filtered_danse_semisupervised[0,:,:,:], 0), offset=0, dim1=1,dim2=2).numpy()) if "danse_semisupervised" in models_list else None,
        savefig=True,
    ...
    plot_meas_trajectory_w_lims(
        ...
        Y_pred_SemiDANSE=torch.squeeze(Y_estimated_pred_danse_semisupervised[0], 0).numpy(), 
        Y_pred_SemiDANSE_std=np.sqrt(torch.diagonal(torch.squeeze(Py_estimated_pred_danse_semisupervised[0], 0), offset=0, dim1=1,dim2=2).numpy()), 
        savefig=True,
...
if __name__ == "__main__":
    ...
    list_of_models_comparison = ["ls", "ekf", "ukf", "danse", "danse_semisupervised"]
    ...
    for key in model_file_saved_dict.keys():
        for j, smnr_dB_label in enumerate(smnr_dB_dict_arr):
            ...
            if saved_model == "danse_semisupervised" and nsup is not None:
                ...
                model_file_saved_dict[key][smnr_dB_label] = glob.glob("./models/*{}_{}_*m_{}_*n_{}_*T_{}_N_{}*sigmae2_{}dB_smnr_{}dB*/*best*".format(
                    ssmtype, saved_model, m, n, T_train, N_train_actual, sigma_e2_dB_test, smnr_dB_arr[j]))[-1]
            else:
                model_file_saved_dict[key][smnr_dB_label] = glob.glob("./models/*{}_{}_*opt_gru_m_{}_*n_{}_*T_{}_N_{}*sigmae2_{}dB_smnr_{}dB*/*best*".format(
                    ssmtype, saved_model, m, n, T_train, N_train_actual, sigma_e2_dB_test, smnr_dB_arr[j]))[-1]
