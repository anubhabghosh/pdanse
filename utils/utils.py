...
def my_collate_fn(batch):
    ...
    measurements = [item["measurements"] for item in batch]
    noise_covs = [item["noise_covs"] for item in batch]
    states_tensor = torch.from_numpy(np.row_stack(states))
    measurements_tensor = torch.from_numpy(np.row_stack(measurements))
