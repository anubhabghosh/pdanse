...
class RNN_model(nn.Module):
    ...
    def forward(self, x):
        ...
        # Means and variances are computed for time instants t=1, ..., T using the available sequence
        mu_1T = self.fc_mean(y) # A second linear projection to get the means
        vars_1T = F.softplus(self.fc_vars(y)) # A second linear projection followed by an activation function to get variances

        return mu_1T, vars_1T