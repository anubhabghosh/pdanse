...
class DMM(nn.Module):
    ...
    def forward_emission_net(self, latent_seq_current, Cw_batch=None):
        ...
        if self.emission_use_binary_obs == True:
            ...
            if self.train_emission == True:
                ...
                if Cw_batch is None:
                    ...
                    batch_size, _, _ = Cw_batch.shape
                    var_obs_given_latent_seq_curr = torch.repeat_interleave(
                            torch.diagonal(Cw_batch,0,-1).reshape((batch_size,1,-1)), latent_seq_current.shape[1], dim=1
                        )
...
def train_dmm(model, options, optimizer_params, train_loader, val_loader, logfile_path, 
    ...
    try:
        for epoch in range(nepochs):
            ...
            for i, tr_data in enumerate(train_loader, 0):
            
                tr_Y_batch, tr_X_batch, tr_Cw_batch = tr_data
                optimizer.zero_grad()
            ...
            with torch.no_grad():
                ...
                for i, val_data in enumerate(val_loader, 0):
                    ...
                    val_Y_batch, val_X_batch, val_Cw_batch = val_data
