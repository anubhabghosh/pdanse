...
import math
import os
from utils.utils import compute_log_prob_normal, split_joint_dataset, create_diag, compute_inverse, count_params, ConvergenceMonitor
...
class SemiDANSE(nn.Module):
    ...
    def __init__(self, n_states, n_obs, mu_w, C_w, H, mu_x0, C_x0, batch_size, rnn_type, rnn_params_dict, kappa=0.2, device='cpu'):
    ...
    def forward(self, Yi_batch_unsup, Yi_batch_sup, Xi_batch_sup, Cw_batch_unsup, Cw_batch_sup, use_sup_loss=False, use_unsup_loss=True):
        
        # Compute the unsupervised loss
        if use_unsup_loss == True:
            
            mu_batch_unsup, vars_batch_unsup = self.rnn.forward(x=Yi_batch_unsup)
            mu_xt_yt_prev_unsup, L_xt_yt_prev_unsup = self.compute_prior_mean_vars(mu_xt_yt_prev=mu_batch_unsup, L_xt_yt_prev=vars_batch_unsup)
            self.compute_marginal_mean_vars(mu_xt_yt_prev=mu_xt_yt_prev_unsup, L_xt_yt_prev=L_xt_yt_prev_unsup, Cw_batch=Cw_batch_unsup)
            ...
            if use_sup_loss == True:
                # Compute the supervised loss
                mu_batch_sup, vars_batch_sup = self.rnn.forward(x=Yi_batch_sup)
                mu_xt_yt_prev_sup, L_xt_yt_prev_sup = self.compute_prior_mean_vars(mu_xt_yt_prev=mu_batch_sup, L_xt_yt_prev=vars_batch_sup)
                self.compute_marginal_mean_vars(mu_xt_yt_prev=mu_xt_yt_prev_sup, L_xt_yt_prev=L_xt_yt_prev_sup, Cw_batch=Cw_batch_sup)
                mu_xt_yt_current_sup, L_xt_yt_current_sup = self.compute_posterior_mean_vars(Yi_batch=Yi_batch_sup, Cw_batch=Cw_batch_sup)
                logprob_batch_sup = self.compute_logpdf_Gaussian_X(X=Xi_batch_sup)
                log_pXTYT_batch_avg = logprob_batch_unsup.mean(0) + logprob_batch_sup.mean(0)
...
def train_danse_semisupervised(model, options, train_loader_unsup, val_loader_unsup, train_loader_sup, val_loader_sup, nepochs, logfile_path, modelfile_path, save_chkpoints, device='cpu', tr_verbose=False):
    ...
    if save_chkpoints == "all" or save_chkpoints == "some":
        ...
        if logfile_path is None:
            training_logfile = "./log/danse_semisupervised_{}.log".format(model.rnn_type)
        else:
        ...
        # Grid search
        if logfile_path is None:
            training_logfile = "./log/gs_training_danse_semisupervised_{}.log".format(model.rnn_type)
        else:
            training_logfile = logfile_path
    ...
    try:
        for epoch in range(nepochs):
            ...
            for i, (tr_data_sup, tr_data_unsup) in enumerate(zip(cycle(train_loader_sup), train_loader_unsup)):
                ...
                Y_train_batch_sup = Variable(tr_Y_batch_sup, requires_grad=False).type(torch.FloatTensor).to(device)
                Cw_train_batch_sup = Variable(tr_Cw_batch_sup, requires_grad=False).type(torch.FloatTensor).to(device)
                X_train_batch_sup = Variable(tr_X_batch_sup[:, :, :], requires_grad=False).type(torch.FloatTensor).to(device)

                Y_train_batch_unsup = Variable(tr_Y_batch_unsup, requires_grad=False).type(torch.FloatTensor).to(device)
                Cw_train_batch_unsup = Variable(tr_Cw_batch_unsup, requires_grad=False).type(torch.FloatTensor).to(device)
                X_train_batch_unsup = Variable(tr_X_batch_unsup[:, :, :], requires_grad=False).type(torch.FloatTensor).to(device)

                if Y_train_batch_unsup.shape[0] > 0 and Y_train_batch_sup.shape[0] > 0:
                    log_pXY_train_batch_avg = -model.forward(Y_train_batch_unsup, Y_train_batch_sup, X_train_batch_sup, Cw_train_batch_unsup, Cw_train_batch_sup, use_sup_loss_train)
                elif Y_train_batch_unsup.shape[0] == 0 and Y_train_batch_sup.shape[0] > 0:
                    log_pXY_train_batch_avg = -model.forward(Y_train_batch_unsup, Y_train_batch_sup, X_train_batch_sup, Cw_train_batch_unsup, Cw_train_batch_sup, use_sup_loss_train, use_unsup_loss=False)
                elif Y_train_batch_unsup.shape[0] > 0 and Y_train_batch_sup.shape[0] == 0:
                    log_pXY_train_batch_avg = -model.forward(Y_train_batch_unsup, Y_train_batch_sup, X_train_batch_sup, Cw_train_batch_unsup, Cw_train_batch_sup, use_sup_loss=False)
            ...
            with torch.no_grad():
                ...
                for i, (val_data_sup, val_data_unsup) in enumerate(zip(cycle(val_loader_sup), val_loader_unsup)):
                    
                    if i+1 > n_batches_sup_val:
                        use_sup_loss_val = False

                    val_Y_batch_sup, val_X_batch_sup, val_Cw_batch_sup = val_data_sup
                    val_Y_batch_unsup, val_X_batch_unsup, val_Cw_batch_unsup = val_data_unsup

                    Y_val_batch_sup = Variable(val_Y_batch_sup, requires_grad=False).type(torch.FloatTensor).to(device)
                    X_val_batch_sup = Variable(val_X_batch_sup[:, :, :], requires_grad=False).type(torch.FloatTensor).to(device)
                    Cw_val_batch_sup = Variable(val_Cw_batch_sup, requires_grad=False).type(torch.FloatTensor).to(device)

                    Y_val_batch_unsup = Variable(val_Y_batch_unsup, requires_grad=False).type(torch.FloatTensor).to(device)
                    X_val_batch_unsup = Variable(val_X_batch_unsup[:, :, :], requires_grad=False).type(torch.FloatTensor).to(device)
                    Cw_val_batch_unsup = Variable(val_Cw_batch_unsup, requires_grad=False).type(torch.FloatTensor).to(device)

                    val_mu_X_predictions_batch, val_var_X_predictions_batch, val_mu_X_filtered_batch, val_var_X_filtered_batch = model.compute_predictions(Y_val_batch_unsup, Cw_val_batch_unsup)

                    log_pXY_val_batch_avg = -model.forward(Y_val_batch_unsup, Y_val_batch_sup, X_val_batch_sup, Cw_val_batch_unsup, Cw_val_batch_sup, use_sup_loss_val)
                    val_loss_epoch_sum += log_pXY_val_batch_avg.item()

                    val_mse_loss_batch = mse_criterion(X_val_batch_unsup.to(device), val_mu_X_filtered_batch)
                    # print statistics
                    val_mse_loss_epoch_sum += val_mse_loss_batch.item()

            # Loss at the end of each epoch
            tr_loss = tr_loss_epoch_sum / (len(train_loader_unsup) + len(train_loader_sup))
            val_loss = val_loss_epoch_sum / (len(val_loader_unsup) + len(val_loader_sup))
            val_mse_loss = val_mse_loss_epoch_sum / (len(val_loader_unsup) + len(val_loader_sup))
            ...
            if (((epoch + 1) % 100) == 0 or epoch == 0) and save_chkpoints == "all": 
                ...
                save_model(model, model_filepath + "/" + "danse_semisupervised_{}_ckpt_epoch_{}.pt".format(model.rnn_type, epoch+1))
        ...
        if save_chkpoints == "all" or save_chkpoints == "some":
            ...
            if not best_model_wts is None:
                model_filename = "danse_semisupervised_{}_ckpt_epoch_{}_best.pt".format(model.rnn_type, best_val_epoch)
                torch.save(best_model_wts, model_filepath + "/" + model_filename)
            else:
                model_filename = "danse_semisupervised_{}_ckpt_epoch_{}_best.pt".format(model.rnn_type, epoch+1)
        ...
        if not save_chkpoints is None:
            model_filename = "danse_semisupervised_{}_ckpt_epoch_{}_latest.pt".format(model.rnn_type, epoch+1)
