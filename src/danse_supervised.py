...
class DANSE_Supervised(nn.Module):
    ...
    def forward(self, Yi_batch, Xi_batch, Cw_batch):
        ...
        mu_xt_yt_current_test, L_xt_yt_current_test = self.compute_posterior_mean_vars(Yi_batch=Yi_batch, Cw_batch=Cw_batch)
        ...
        logprob_batch = self.compute_logpdf_Gaussian(X=Xi_batch) / (Xi_batch.shape[1] * Xi_batch.shape[2])
    ...
    try:
        for epoch in range(nepochs):
            ...
            for i, data in enumerate(train_loader, 0):
                ...
                log_pXY_train_batch = -model.forward(Y_train_batch, X_train_batch, Cw_train_batch)
            ...
            with torch.no_grad():
                ...
                for i, data in enumerate(val_loader, 0):
                    ...
                    log_pY_val_batch = -model.forward(Y_val_batch, X_val_batch, Cw_val_batch)
            ...
            if (((epoch + 1) % 100) == 0 or epoch == 0) and save_chkpoints == "all": 
                ...
                save_model(model, model_filepath + "/" + "danse_supervised_{}_ckpt_epoch_{}.pt".format(model.rnn_type, epoch+1))
        ...
        if not save_chkpoints is None:
            model_filename = "danse_supervised_{}_ckpt_epoch_{}_latest.pt".format(model.rnn_type, epoch+1)
