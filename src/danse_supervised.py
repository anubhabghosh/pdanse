...
class DANSE_Supervised(nn.Module):
    ...
    def compute_predictions(self, Yi_test_batch, Cw_test_batch):
        ...
        log_post_weights_seq_for_mean = log_post_weights_seq.unsqueeze(3).repeat(
            1, 1, 1, xt_yt_prev_test_expanded.shape[-1]
        )
        log_post_weights_seq_for_cov = (
            log_post_weights_seq.unsqueeze(3)
            .unsqueeze(4)
            .repeat(
                1,
                1,
        ...
        self.mu_xt_yt_current = (
            log_post_weights_seq_for_mean.exp() * xt_yt_prev_test_expanded
        ).sum(0)
        ...
        self.residual_xt_yt_current = (
            xt_yt_prev_test_expanded
            - self.mu_xt_yt_current.unsqueeze(0).repeat(self.n_MC, 1, 1, 1)
        )
    ...
    def forward(self, Yi_batch_sup, Xi_batch_sup, Cw_batch_sup):
...
def train_danse_supervised_plus(
    model,
    options,
    train_loader_sup,
    val_loader_sup,
    ...
    if save_chkpoints == "all" or save_chkpoints == "some":
        ...
        if logfile_path is None:
            training_logfile = "./log/danse_supervised_{}.log".format(
                model.rnn_type
            )
        else:
        ...
        # Grid search
        if logfile_path is None:
            training_logfile = "./log/gs_training_danse_supervised_{}.log".format(
                model.rnn_type
            )
        else:
            training_logfile = logfile_path
    ...
    try:
        for epoch in range(nepochs):
            ...
            for i, tr_data in tr_loader_iterator:
                ...
                log_pXY_train_batch_avg = -model.forward(
                    Y_train_batch_sup, X_train_batch_sup, Cw_train_batch_sup
                )
            ...
            with torch.no_grad():
                for i, val_data in val_loader_iterator:
                    ...
                    log_pXY_val_batch_avg = -model.forward(
                        Y_val_batch_sup, X_val_batch_sup, Cw_val_batch_sup
                    )
            ...
            tr_loss = tr_loss_epoch_sum / (len(train_loader_sup))
            val_loss = val_loss_epoch_sum / (len(val_loader_sup))
            val_mse_loss = val_mse_loss_epoch_sum / (len(val_loader_sup))
            ...
            if (((epoch + 1) % 100) == 0 or epoch == 0) and save_chkpoints == "all":
                ...
                save_model(
                    ...
                    model_filepath
                    + "/"
                    + "danse_supervised_{}_ckpt_epoch_{}.pt".format(
        ...
        if save_chkpoints == "all" or save_chkpoints == "some":
            ...
            if best_model_wts is not None:
                model_filename = "danse_supervised_{}_ckpt_epoch_{}_best.pt".format(
        ...
        if save_chkpoints is not None:
            model_filename = "danse_supervised_{}_ckpt_epoch_{}_latest.pt".format(
                model.rnn_type, epoch + 1
            )
