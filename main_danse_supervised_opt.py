...
def main():
    ...
    parser.add_argument("--n_train", help="Pre-specify number of training samples to run the algorithm on, in case of experiment with different N", type=int, default=-1)
    ...
    # In it is pre-specified to use a specific number of samples, else the full dataset is used
    if n_train_reqd > 0:
        
        print("Taking {} samples of the original dataset".format(n_train_reqd))
        Z_XY_partitioned_dict = Z_XY.copy()
        idx_shuffle_all = np.arange(Z_XY_partitioned_dict['data'].shape[0])
        np.random.shuffle(idx_shuffle_all)
        idx_train_reqd = idx_shuffle_all[:n_train_reqd]
        print("Training indices: {}".format(idx_train_reqd))
        Z_XY_partitioned_dict['data'] = Z_XY_partitioned_dict['data'][idx_train_reqd, :]
        Z_XY_partitioned_dict['trajectory_lengths'] = Z_XY_partitioned_dict['trajectory_lengths'][idx_train_reqd, :]
        Z_XY_partitioned_dict['num_samples'] = Z_XY_partitioned_dict['data'].shape[0]
        Z_XY_dataset = Series_Dataset(Z_XY_dict=Z_XY_partitioned_dict)
        N_samples = copy.deepcopy(n_train_reqd)
    ...
    if not os.path.isfile(splits_file) or n_train_reqd > 0 or n_train_reqd == -1:
        ...
        print(tr_indices, val_indices, test_indices)
        print(len(tr_indices), len(val_indices), len(test_indices))

    if n_train_reqd > 0:
        batch_size = get_batch_size_sup(N=len(tr_indices))
        val_batch_size = get_batch_size_sup(N=len(val_indices))
        test_batch_size = get_batch_size_sup(N=len(test_indices))
    else:
        val_batch_size = None
    ...
    #print("Dataset sizes: train - {}, val - {}, test-  {}".format(len(train_loader.dataset), len(val_loader.dataset), len(test_loader.dataset)))
    print("No. of training, validation and testing batches: {}, {}, {}".format(len(train_loader), 
                                                                                len(val_loader), 
                                                                                len(test_loader)))
    
    print("Training, validation and testing batch sizes: {}, {}, {}".format(train_loader.batch_size, 
